	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Los detalles del usuario se guardaron con éxito.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
				...
				if(!empty($_FILES['img']['tmp_name'])){
					...
					if(!in_array($_FILES['img']['type'],$accept)){
						$err = "El tipo de archivo de imagen no es válido";
					}
