        ...
        <div class="card rounded-0">
            <div class="card-body">
                <div class="w-100 justify-content-between d-flex">
                    <h4><b>Órdenes</b></h4>
                    <a href="./?p=edit_account" class="btn btn btn-default btn-flat bg-maroon"><div class="fa fa-user-cog"></div> Gestionar Cuenta</a>
                </div>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                            <?php if($row['status'] == 0): ?>
                                                <span class="badge badge-light text-dark border px-3 rounded-pill">Pendiente</span>
                                            <?php elseif($row['status'] == 1): ?>
                                                <span class="badge badge-primary px-3 rounded-pill">Empacado</span>
                                            <?php elseif($row['status'] == 2): ?>
                                                <span class="badge badge-warning px-3 rounded-pill">Disponible para Entrega</span>
        ...
        $.ajax({
            ...
            success:function(resp){
                if(typeof resp == 'object' && resp.status == 'success'){
                    alert_toast("Reserva cancelada con éxito",'success')
                    setTimeout(function(){
                        location.reload()
                    },2000)
                }else{
