					...
					?>
						<tr>
							...
							<td class="text-center">
                                ...
                                <?php else: ?>
                                    <span class="badge badge-success px-2 rounded-pill">Sí</span>
                                <?php endif; ?>
                            </td>
							<td class="text-center">
                                <?php if($row['status'] == 0): ?>
                                    <span class="badge badge-light border px-3 rounded-pill">Pendiente</span>
                                <?php elseif($row['status'] == 1): ?>
                                    <span class="badge badge-primary px-3 rounded-pill">Empacado</span>
								<?php elseif($row['status'] == 2): ?>
                                    <span class="badge badge-warning px-3 rounded-pill">Para Envío</span>
								<?php elseif($row['status'] == 3): ?>
                                    <span class="badge badge-success px-3 rounded-pill">Entregado</span>
                                <?php else: ?>
							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
				                    <a class="dropdown-item" href="?page=orders/view_order&id=<?php echo $row['id'] ?>">Ver Orden</a>
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Deseas eliminar esta orden permanentemente?","delete_order",[$(this).attr('data-id')])
		})
		$('.pay_order').click(function(){
			_conf("Deseas marcar esta orden como pagada?","pay_order",[$(this).attr('data-id')])
		})
		$('.table').dataTable();
	})
